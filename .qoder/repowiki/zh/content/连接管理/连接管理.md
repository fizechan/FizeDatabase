...
**所做更改**
- 新增了完整的连接管理文档框架
- 详细说明了PDO、ODBC、ADODB三种连接模式的实现原理
- 补充了连接工厂的创建流程和配置方法
- 添加了连接池管理、连接生命周期控制等高级功能说明
- 提供了不同连接模式的性能对比和选择指导
- 包含了实际的配置示例和故障排除方法
...
- [src/Extend/PgSQL/Mode.php:14-59](file://src/Extend/PgSQL/Mode.php#L14-L59)
- [src/Extend/SQLSRV/Mode.php:13-80](file://src/Extend/SQLSRV/Mode.php#L13-L80)
...
- [src/Extend/PgSQL/Mode/PDOMode.php:1-45](file://src/Extend/PgSQL/Mode/PDOMode.php#L1-L45)
- [src/Extend/SQLSRV/Mode/PDOMode.php:1-61](file://src/Extend/SQLSRV/Mode/PDOMode.php#L1-L61)
...
**图表来源**
- [src/Extend/Access/Mode/ODBCMode.php:1-60](file://src/Extend/Access/Mode/ODBCMode.php#L1-L60)
- [src/Extend/SQLSRV/Mode/ODBCMode.php:1-43](file://src/Extend/SQLSRV/Mode/ODBCMode.php#L1-L43)
...
### PostgreSQL PgSQL 模式
- 通过原生 PgSQL 扩展连接 PostgreSQL，支持连接字符串、长连接与连接类型；查询/执行过程与 PDO 类似。
- 特殊点：支持 pconnect 参数控制长连接；lastInsertId 通过序列名获取。

```mermaid
classDiagram
class PgSQLMode {
+__construct(connection_string,pconnect,connect_type)
+query(sql,params, callback) array
+execute(sql,params) int
+lastInsertId(name) int|string
+__destruct()
...
**图表来源**
- [src/Extend/PgSQL/Mode/PgSQLMode.php:1-57](file://src/Extend/PgSQL/Mode/PgSQLMode.php#L1-L57)

**章节来源**
- [src/Extend/PgSQL/Mode/PgSQLMode.php:1-57](file://src/Extend/PgSQL/Mode/PgSQLMode.php#L1-L57)

### SQL Server SQLSRV 模式
- 通过微软官方 SQLSRV 扩展连接 SQL Server，支持高性能连接与丰富的配置选项；查询/执行过程与 PDO 类似。
- 特殊点：支持 newFeature 参数控制新特性；字符集默认为 GBK。

```mermaid
classDiagram
class SQLSRVMode {
+__construct(host,user,pwd,dbname,port,charset)
+query(sql,params, callback) array
+execute(sql,params) int
+__destruct()
}
SQLSRVMode --> "继承" CoreDb
```

**图表来源**
- [src/Extend/SQLSRV/Mode/SQLSRVMode.php:1-78](file://src/Extend/SQLSRV/Mode/SQLSRVMode.php#L1-L78)
...
- PgSQL：构造时建立连接，析构时释放；支持长连接控制。
- SQLSRV：构造时建立连接，析构时释放；支持新特性开关。
...
- [src/Extend/PgSQL/Mode/PgSQLMode.php:14-20](file://src/Extend/PgSQL/Mode/PgSQLMode.php#L14-L20)
- [src/Extend/SQLSRV/Mode/SQLSRVMode.php:14-20](file://src/Extend/SQLSRV/Mode/SQLSRVMode.php#L14-L20)
...
Db --> MF_PG["PgSQL/ModeFactory"]
Db --> MF_SQLSRV["SQLSRV/ModeFactory"]
MF_My --> M_My["MySQL/Mode"]
MF_Acc --> M_Acc["Access/Mode"]
MF_Orcl --> M_Orcl["Oracle/Mode"]
MF_PG --> M_PG["PgSQL/Mode"]
MF_SQLSRV --> M_SQLSRV["SQLSRV/Mode"]
...
M_PG --> PM_PG_PgSQL["PgSQL/PgSQLMode"]
M_SQLSRV --> PM_SQLSRV_SQLSRV["SQLSRV/SQLSRVMode"]
```
...
- PgSQL：原生扩展支持长连接控制，适合高并发场景。
- SQLSRV：微软官方支持，性能优异，适合 Windows 环境下的 SQL Server 部署。
...
- [src/Extend/SQLSRV/Mode.php:45-78](file://src/Extend/SQLSRV/Mode.php#L45-L78)
...
- 连接失败
  ...
  - PgSQL：检查连接字符串格式与长连接参数。
  - SQLSRV：检查驱动版本与新特性配置。
- 字符集问题
  - Access 的 PDO 模式涉及 UTF-8 与 GBK 转换，确保 SQL 与参数在 prepare 前后正确编码。
  - SQL Server 默认字符集为 GBK，注意与其他数据库的差异。
- 事务异常
  - 确认嵌套事务计数与实际提交/回滚时机；避免在非最外层误触发 commit/rollback。
- 资源泄漏
  - 确保具体模式类的析构被调用（如 PDO/ODBC/ADODB/MySQLi/OCI/PgSQL/SQLSRV），避免连接未释放导致资源耗尽。
- 性能问题
  - 检查连接池配置与连接复用策略。
  - 对于高并发场景，考虑使用 PDO 的持久化连接或 PgSQL 的长连接。
...
## 结论
FizeDatabase 通过"核心抽象 + 扩展工厂 + 模式工厂 + 具体模式类"的分层设计，实现了对 PDO、ODBC、ADODB 等多种连接模式的统一接入与差异化实现。PDO 模式在跨平台与生态方面具有明显优势，是大多数数据库的首选；ODBC 适合通用桥接场景；ADODB 适合 Access 文件数据库；MySQLi 与 OCI 分别面向 MySQL 与 Oracle 的特定需求；PgSQL 原生扩展适合 PostgreSQL；SQLSRV 适合 Windows 环境下的 SQL Server。结合事务嵌套、查询构建与资源释放策略，可在保证稳定性的同时兼顾性能与可维护性。
...
- SQL Server（PDO/ODBC/ADODB/SQLSRV）
  - 关键参数：host、user、password、dbname、port、charset、driver、new_feature
  - 建议：明确驱动与字符集；ODBC/ADODB 需正确配置驱动；PDO 注意字符集转换；SQLSRV 使用微软官方驱动
- PostgreSQL（PDO/ODBC/PgSQL）
  - 关键参数：host、user、password、dbname、port、charset、driver、pconnect、connect_type
  - 建议：合理设置长连接；注意连接字符串格式；ODBC 需正确配置驱动
- 连接池管理
  - 建议：对于高并发场景，使用 PDO 的持久化连接或 PgSQL 的长连接
  - 监控：定期检查连接状态与资源使用情况
  - 超时：设置合理的连接超时与查询超时时间
- 重连机制
  - 建议：在应用层实现连接检测与自动重连逻辑
  - 错误处理：捕获连接异常并进行优雅降级
- 状态监控
  - 建议：记录连接建立、执行时间、错误日志等指标
  - 告警：设置连接池使用率告警阈值
...
- [src/Extend/PgSQL/ModeFactory.php:21-55](file://src/Extend/PgSQL/ModeFactory.php#L21-L55)
- [src/Extend/SQLSRV/ModeFactory.php:23-54](file://src/Extend/SQLSRV/ModeFactory.php#L23-L54)
- [src/Extend/MySQL/Mode.php:18-72](file://src/Extend/MySQL/Mode.php#L18-L72)
- [src/Extend/Oracle/Mode.php:18-61](file://src/Extend/Oracle/Mode.php#L18-L61)
- [src/Extend/Access/Mode.php:16-49](file://src/Extend/Access/Mode.php#L16-L49)
- [src/Extend/PgSQL/Mode.php:18-57](file://src/Extend/PgSQL/Mode.php#L18-L57)

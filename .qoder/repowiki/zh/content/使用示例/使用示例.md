...
AccessQuery["Access Query 实现<br/>src/Extend/Access/Query.php"]
SQLiteDb["SQLite Db 实现<br/>src/Extend/SQLite/Db.php"]
SQLiteQuery["SQLite Query 实现<br/>src/Extend/SQLite/Query.php"]
end
APP --> DBFacade
APP --> QueryFacade
DBFacade --> CoreDb
...
CoreQuery --> AccessQuery
CoreQuery --> SQLiteQuery
```
...
#### 多表关联查询示例
- 场景描述
  - 用户订单详情查询，包含用户信息、商品信息、订单状态
- 实现要点
  - 使用 LEFT JOIN 处理可能缺失的信息
  - 使用 WHERE 条件进行精确筛选
  - 使用 ORDER BY 按时间倒序排列

#### 统计分析查询示例
- 场景描述
  - 按月统计销售数据，包含销售额、订单数、用户数
- 实现要点
  - 使用 GROUP BY 按月份分组
  - 使用聚合函数计算统计指标
  - 使用 HAVING 进行分组筛选
...
#### 嵌套事务管理
- 实现要点
  - 使用 startTrans/commit/rollback 管理事务层级
  - 确保外层事务控制整体流程
  - 正确处理异常情况下的回滚

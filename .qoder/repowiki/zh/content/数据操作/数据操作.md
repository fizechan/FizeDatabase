...
**更新** 本次更新重点加强了表达式更新、批量操作和事务管理的详细说明，为开发者提供更全面的数据操作指导。
...
### 表达式更新与高级更新操作
FizeDatabase 提供了强大的表达式更新能力，支持原样 SQL 表达式写入和字段自增自减操作：

- **原样 SQL 表达式更新**
  - 支持直接写入原生 SQL 表达式，如 `['login_count' => ['login_count + 1']]`
  - 通过数组形式的值实现复杂计算逻辑
  - 完全由开发者控制 SQL 语句，适合复杂的数学运算和函数调用

- **字段自增自减**
  - `setInc(field, step=1)`：字段值递增指定步长
  - `setDec(field, step=1)`：字段值递减指定步长
  - 自动处理字段名格式化和 SQL 构建

- **设置任意字段值**
  - `setValue(field, value)`：设置指定字段的任意值
  - 支持常量值、表达式或数组形式的原样写入

**章节来源**
- [src/Core/Db.php:847-881](file://src/Core/Db.php#L847-L881)
- [examples/db_update.php:15-22](file://examples/db_update.php#L15-L22)
...
## 结论
FizeDatabase 通过"门面 + 核心抽象 + 扩展 + 模式 + 查询器"的架构，提供了统一、安全、高性能的数据操作体验。其参数化查询、丰富的条件构建、事务嵌套、分页与批量插入等能力，既适合简单场景也适用于复杂业务。新增的表达式更新功能进一步增强了数据操作的灵活性，支持复杂的数学运算和函数调用。建议在生产环境遵循参数绑定、查询缓存与事务最小化原则，结合具体数据库方言特性（如 MySQL 的 REPLACE/LOCK/Paginate）获得最佳性能与稳定性。
...
- 高级更新操作
  - CoreDb::setValue(field, value)
  - CoreDb::setInc(field, step=1)
  - CoreDb::setDec(field, step=1)
- 事务

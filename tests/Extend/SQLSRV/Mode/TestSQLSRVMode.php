...
namespace Tests\Extend\SQLSRV\Mode;

use Fize\Database\Extend\SQLSRV\Mode\SQLSRVMode;
use PHPUnit\Framework\TestCase;

class TestSQLSRVMode extends TestCase
{

    public function test__construct()
    {
        $db = new SQLSRVMode('(local)', 'sa', '123456', 'gm_test');
        var_dump($db);
    ...
    public function testQuery()
    {
        ...
        //改
        $sql = 'UPDATE [user] SET [name] = ? WHERE id = 20';
    ...
    public function testCommit()
    {
        ...
        $sql = 'UPDATE [user] SET [name] = ? WHERE id = 16';
        ...
        $sql = 'SELECT * FROM [user] WHERE id = 16';
        $rows = $db->query($sql);
        var_dump($rows[0]);

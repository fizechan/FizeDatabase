<?php

namespace middleware\driver\odbc;

use fize\database\driver\odbc\Statement;
use PHPUnit\Framework\TestCase;

class TestStatement extends TestCase
{

    public function testFieldLen()
    {

    }

    public function testFieldPrecision()
    {

    }

    public function testResult()
    {

    }

    public function testFetchRow()
    {

    }

    public function testFieldName()
    {

    }

    public function testLongreadlen()
    {

    }

    public function testFieldType()
    {

    }

    public function testExecute()
    {

    }

    public function testSetoption()
    {

    }

    public function testResultAll()
    {

    }

    public function testNextResult()
    {

    }

    public function testFieldScale()
    {

    }

    public function testFetchArray()
    {

    }

    public function testFetchInto()
    {

    }

    public function testNumFields()
    {

    }

    public function testCursor()
    {

    }

    public function testFieldNum()
    {

    }

    public function testFetchObject()
    {

    }

    public function testNumRows()
    {

    }

    public function test__construct()
    {

    }

    public function testBinmode()
    {

    }

    public function testFreeResult()
    {

    }
}

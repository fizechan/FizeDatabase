...
namespace extend\pgsql\mode\driver;

use fize\db\extend\pgsql\mode\driver\Pgsql;
...
class TestPgsql extends TestCase
{
    ...
    {
        $db = new Pgsql("host=192.168.56.101 port=5432 dbname=gmtest user=root password=123456");
        $data = file_get_contents(__DIR__ . '/TestPgsql.php');
    ...
    public function testLoExport()
    {
        ...
        $oid = $db->loCreate();
        $lo = $db->loOpen($oid, 'w');
        $lo->loWrite(file_get_contents(__DIR__ . './TestPgsql.php'));
        $lo->loClose();
    ...
    public function testLoImport()
    {
        $db = new Pgsql("host=192.168.56.101 port=5432 dbname=gmtest user=root password=123456");
        $db->query("BEGIN");
        $oid = $db->loImport(__DIR__ . './TestPgsql.php');
        var_dump($oid);
        $db->query("COMMIT");
        self::assertIsInt($oid);
    }
    ...
    public function testLoWrite()
    {
        ...
        $oid = $db->loCreate();
        $lo = $db->loOpen($oid, 'w');
        $count = $lo->loWrite(file_get_contents(__DIR__ . './TestPgsql.php'));
        $lo->loClose();
        $db->query("COMMIT");
        var_dump($count);
        self::assertIsInt($count);

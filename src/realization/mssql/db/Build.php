...
trait Build
{
    ...
    {
        if (!is_null($this->_offset) && !$this->_new_feature) {  // 进行偏移量移动的旧版SQL组装
            ...
            $this->_order = '';  //消除子语句使用ORDER BY语句的错误
            $this->_field .= ", ROW_NUMBER() OVER ({$order}) AS _RN_";
        }
        ...
        if (!empty($this->_size) && empty($this->_offset)) {
            $sql = substr_replace($sql, " TOP {$this->_size}", 6, 0);
        }

        if (!is_null($this->_offset)) {  // 进行偏移量移动
            if ($this->_new_feature) {
                //自MSSQL2012开始支持“OFFSET 1 ROWS FETCH NEXT 3 ROWS ONLY”语句
                if (empty($this->_order)) {  //OFFSET 需要与ORDER BY语句一起使用，如果没有指定ORDER BY 则使用RAND()
                    $sql .= " ORDER BY RAND() OFFSET {$this->_offset} ROWS FETCH NEXT {$this->_size} ROWS ONLY";
                } else {
                    $sql .= " OFFSET {$this->_offset} ROWS FETCH NEXT {$this->_size} ROWS ONLY";
                }
            } else {
                $sql = "SELECT TOP {$this->_size} * FROM ({$sql}) AS _TT_ WHERE _TT_._RN_ > {$this->_offset}";
            }
        }
